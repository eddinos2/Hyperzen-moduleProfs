...
async function createProfessors() {
  console.log('🎓 Création de 180 professeurs avec Docker local...');
  ...
  // Créer les professeurs un par un
  for (let i = 0; i < professors.length; i++) {
    const prof = professors[i];
    
    if (i % 20 === 0) {
      console.log(`📦 Progression: ${i + 1}/${professors.length}`);
    }
    ...
    try {
      ...
      if (profileError) {
        ...
        console.log(`✅ Professeur créé: ${prof.firstName} ${prof.lastName} (${prof.email})`);
        successCount++;
      }
    } catch (error) {
