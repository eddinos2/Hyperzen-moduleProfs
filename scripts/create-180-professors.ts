...
async function createProfessors() {
  ...
  const professors = [];
  const batchSize = 20; // Traiter par lots de 20 pour éviter les timeouts
  ...
  for (let i = 0; i < professors.length; i += batchSize) {
    ...
    for (const prof of batch) {
      try {
        // Créer l'utilisateur avec la fonction RPC
        const { data: userData, error: userError } = await supabase
          .rpc('create_user_profile', {
            p_email: prof.email,
            p_password: prof.password,
            p_first_name: prof.firstName,
            p_last_name: prof.lastName,
            p_role: 'ENSEIGNANT',
            p_campus_id: prof.campusId
          });

        if (userError) {
          console.error(`❌ Erreur création ${prof.email}:`, userError.message);
        } else {
  ...
  console.log('🎉 Création des professeurs terminée !');
